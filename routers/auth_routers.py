...
from databases.database import users_db, refresh_tokens_db, email_verification_db, password_reset_db
import uuid
import random
...
def verify_email_code(code: str = Body(..., embed=True)):
    for user_id, record in email_verification_db.items():
...
def confirm_password_reset(data: PasswordResetConfirm):
    for user_id, record in password_reset_db.items():
        if record["code"] == data.token:
